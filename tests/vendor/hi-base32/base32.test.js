// From https://github.com/emn178/hi-base32/blob/master/tests/test.js

import { describe, test as it } from 'node:test'
import { toBase32, fromBase32 } from '@exodus/bytes/base32.js'

const base32 = {
  encode: (x) => toBase32(Buffer.from(x), { padding: true }),
  decode: (x) => fromBase32(x, { format: 'buffer' }).toString(),
}

const expect = (x) => ({
  to: {
    be: (y) => {
      if (x !== y) throw new Error(`Not equal: ${JSON.stringify(x)} != ${JSON.stringify(y)}`)
    },
  },
})
